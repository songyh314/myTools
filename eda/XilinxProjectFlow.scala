...
${if (xcix != null) {
        xcix.map{item => s"add_files {$item}"}.mkString("\n")
      } else ""}
import_files -force
...
"""
    ...
    xdc.write(s"""create_clock -name clk -period ${(targetPeriod * 1e9) toBigDecimal} [get_ports clk]""")
    xdc.write(s"\n")
    xdc.write(
      s"""set_input_delay ${(targetPeriod * 1e8) toBigDecimal} -clock [get_clocks clk] [get_ports -filter {DIRECTION == IN && NAME !~ "rst"}]"""
    )
    xdc.write(s"\n")
    xdc.write(
      s"""set_output_delay ${(targetPeriod * 1e8) toBigDecimal} -clock [get_clocks clk] [get_ports -filter {DIRECTION == OUT}]"""
    )
    xdc.flush();
    xdc.close();
